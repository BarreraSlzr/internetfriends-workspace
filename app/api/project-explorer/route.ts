...
class ServerMicroUXExplorer {
  ...
  private createEvent(type: ExplorerEvent["type"],
    data: unknown,)
    transportable = true,)
  ): ExplorerEvent {
  ...
  private generateGraphData(nodes: ProjectNode[]): unknown {
    ...
    const processNode = (node: ProjectNode, parentId?: string) => {
      ...
      graphNodes.push({
        ...
        group: node.type,
        importance: node.metadata.importance,
        size: node.metadata.importance === "critical"
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (targetFiles && Array.isArray(targetFiles)) {
      analysis.transportable_ui_data.tree_view =
        analysis.transportable_ui_data.tree_view.filter((__event: Event) =>

          targetFiles.some((target) => node.label.includes(target)),
        );
      analysis.transportable_ui_data.grid_view =
        analysis.transportable_ui_data.grid_view.filter((__event: Event) =>

          targetFiles.some((target) => node.name.includes(target)),
        );
