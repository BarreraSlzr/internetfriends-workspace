 ...
 *
 * This script provides intelligent development assistance for the InternetFriends project: * - Automated code generation with context awareness
...
class AIDevAssistant {
  ...
  async generateComponent(request: ComponentGenerationRequest): Promise<void> {
    ...
    const componentDir = join(this.config.projectRoot,
      "components",
      request.type,)
      request.name)
    );
...
Commands: generate <type> <name>     Generate a new component
...
Examples: bun scripts/ai-dev-assistant.ts generate atomic Button
...
Options:)
  ...
  try {
    switch (command) {
      ...
      default: console.log("❌ Unknown command. Use --help for usage information.");,
