...
const contentFixes = [
  ...
  {
    pattern: /href=['"]_https?:/g,
    replacement: (match: string) => match.replace("_", ""),
  },
];

// Fix missing keys in map iterations (pattern defined but not used in current implementation)
...
function fixFile(filePath: string): number {
  try {
    ...
    if (content !== originalContent) {
      ...
      console.log(
        `✅ Fixed ${fixCount} React prop issues in ${path.relative(process.cwd(), filePath)}`,
      );
    }
