...
class OrganismValidator {
  ...
  constructor(
    basePath: string = "./app/(internetfriends)/components/organism",
  ...
  private async findOrganisms(): Promise<string[]> {
    try {
      ...
      if (this.verbose) {
        console.log(
          "⚠️  Organism directory not found, creating validation for expected structure...",
        );
      }
  ...
  private async validateOrganism(
    organismName: string,
    ...
    result.score = Math.round(
      (detailScores.filter(Boolean).length / detailScores.length) * 100,
    );
  ...
  private async validateTypeScript(
    ...
    try {
      ...
      const hasProperExport =
        componentContent.includes(`export const`) ||
      ...
      const hasEventIntegration =
        componentContent.includes("UIEvents") ||
      ...
      const hasExportedTypes =
        typesContent.includes("export interface") ||
      ...
      if (
        ...
        if (!hasEventIntegration)
          issues.push("missing event system integration");
  ...
  private async validateStyles(
    ...
    const stylesFile = join(
      organismPath,
      `${result.component}.styles.module.scss`,
    );

    try {
      const stylesContent = await Bun.file(stylesFile).text();

      const hasModularClasses = /\.[a-zA-Z][a-zA-Z0-9_-]*\s*\{/.test(
        stylesContent,
      );
      const hasInternetFriendsVars =
        stylesContent.includes("--if-primary") ||
      ...
      const hasAccessibilityStyles =
        stylesContent.includes("focus-visible") ||
        stylesContent.includes("prefers-reduced-motion");

      if (
        hasModularClasses &&
        hasInternetFriendsVars &&
        hasResponsiveDesign &&
        hasAnimations &&
        hasAccessibilityStyles
      ) {
        result.details.styles = true;
      } else {
        const issues = [];
        if (!hasModularClasses) issues.push("missing CSS module classes");
        if (!hasInternetFriendsVars)
          issues.push("not using InternetFriends design tokens");
        ...
        if (!hasAccessibilityStyles)
          issues.push("missing accessibility styles");
  ...
  private async validateExports(
    ...
    try {
      ...
      const expectedComponentName =
        this.toPascalCase(result.component) + "Organism";
      const hasProperNaming = componentContent.includes(expectedComponentName);

      // Check for default export
      const hasDefaultExport =
        componentContent.includes(`export default ${expectedComponentName}`) ||
        componentContent.includes("export default");

      // Check for named export
      const hasNamedExport = componentContent.includes(
        `export const ${expectedComponentName}`,
      );
      ...
      if (hasProperNaming && (hasDefaultExport || hasNamedExport)) {
        ...
        const issues = [];
        if (!hasProperNaming)
          issues.push(`component should be named ${expectedComponentName}`);
        if (!hasDefaultExport && !hasNamedExport)
          issues.push("missing proper exports");
  ...
  private async validateDocumentation(
    ...
    try {
      ...
      const hasPropDocumentation =
        componentContent.includes("@param") ||
      ...
      if (hasJSDocComments || hasPropDocumentation || hasUsageExamples) {
        ...
        result.issues.push(
          "Missing documentation (JSDoc comments, prop descriptions, or usage examples)",
        );
      }
    } catch {
  ...
  private async validateAccessibility(
    ...
    try {
      ...
      const hasSemanticHTML =
        componentContent.includes("<main>") ||
      ...
      const hasFocusManagement =
        componentContent.includes("focus-visible") ||
      ...
      if (
        ...
        result.issues.push(
          "Missing accessibility features (ARIA attributes, semantic HTML, keyboard support, or focus management)",
        );
      }
    } catch {
  ...
  private generateSummaryReport(): void {
    ...
    console.log(
      `Success Rate: ${((validComponents / totalComponents) * 100).toFixed(1)}%`,
    );
    ...
    categories.forEach((category) => {
      const passed = this.results.filter(
        (r) => r.details[category as keyof typeof r.details],
      ).length;
      const percentage = ((passed / totalComponents) * 100).toFixed(1);
      console.log(
        `  ${category}: ${passed}/${totalComponents} (${percentage}%)`,
      );
    });
    console.log("");
    ...
    if (averageScore < 80) {
      console.log(
        "  - Focus on improving component structure and TypeScript implementation",
      );
    }
    if (this.results.some((r) => !r.details.accessibility)) {
      console.log(
        "  - Add more accessibility features (ARIA attributes, keyboard support)",
      );
    }
    ...
    if (validComponents === totalComponents && averageScore >= 90) {
      ...
      console.log(
        `❌ ${totalComponents - validComponents} component(s) need fixes`,
      );
      process.exit(1);
    }
  }
