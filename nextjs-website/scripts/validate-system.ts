...
class SystemValidator {
  ...
  async validateFileStructure(): Promise<ValidationSuite> {
    ...
    for (const file of requiredFiles) {
      results.push(
        await this.validateTest(`File exists: ${file}`, async () => {
    ...
    for (const dir of requiredDirectories) {
      results.push(
        await this.validateTest(`Directory exists: ${dir}`, async () => {
  ...
  async validatePackageIntegrity(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("package.json is valid JSON", async () => {
        try {
    ...
    results.push(
      await this.validateTest("Required dependencies installed", async () => {
        const { success } = await this.runCommand("bun pm ls", {
          timeout: 10000,
        });
        return success;
      }),
    ...
    results.push(
      await this.validateTest("No security vulnerabilities", async () => {
        const { success, output } = await this.runCommand("bun audit", {
          timeout: 15000,
        });
  ...
  async validateTypeScript(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("TypeScript compilation", async () => {
        const { success, output } = await this.runCommand("bunx tsc --noEmit", {
          timeout: 30000,
        });
    ...
    results.push(
      await this.validateTest("ESLint validation", async () => {
        const { success, output } = await this.runCommand("bunx eslint .", {
          timeout: 20000,
        });
        if (!success && this.verbose) {
  ...
  async validateDesignSystem(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("CSS compilation", async () => {
        const { success } = await this.runCommand(
          "bunx tailwindcss -i ./app/(internetfriends)/globals.css -o ./temp-output.css",
          { timeout: 15000 },
        );
        if (success) {
          // Clean up temp file
    ...
    results.push(
      await this.validateTest("Design tokens present", async () => {
    ...
    results.push(
      await this.validateTest("Component registry functional", async () => {
        try {
          const registryPath =
    ...
    results.push(
      await this.validateTest("Atomic components exportable", async () => {
        try {
  ...
  async validateBuildSystem(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("Next.js build succeeds", async () => {
        const { success, output } = await this.runCommand("bun run build", {
          timeout: 120000,
        });
        if (!success && this.verbose) {
    ...
    results.push(
      await this.validateTest("Build output is valid", async () => {
  ...
  async validateTests(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("Unit tests pass", async () => {
        const { success, output } = await this.runCommand("bun test", {
          timeout: 30000,
        });
        if (!success && this.verbose) {
    ...
    results.push(
      await this.validateTest("Playwright tests pass", async () => {
  ...
  async validateDevelopmentServer(): Promise<ValidationSuite> {
    ...
    results.push(
      await this.validateTest("Server can start", async () => {
  ...
  private generateReport(): void {
    const totalTests = this.results.reduce(
      (acc, suite) => acc + suite.results.length,
      0,
    );
    ...
    this.results.forEach((suite) => {
      ...
      console.log(
        `${icon} ${suite.name}: ${passedCount}/${suite.results.length} (${suite.totalDuration}ms)`,
      );
    ...
    if (allPassed) {
      this.log(
        "\n🎉 All validations passed! System is ready for development.",
        "success",
      );
    } else {
      this.log(
        "\n💥 Some validations failed. Please fix the issues before continuing.",
        "error",
      );

      if (this.fix) {
        this.log(
          "\n🔧 Auto-fix mode enabled. Attempting to fix common issues...",
          "warning",
        );
        this.attemptAutoFix();
      }
