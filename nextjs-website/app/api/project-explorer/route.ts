...
class ServerMicroUXExplorer {
  ...
  private createEvent<T = unknown>(
    ...
    data: T,
    transportable = true,
  ): ExplorerEvent & { data: T } {
    const event: ExplorerEvent & { data: T } = {
      type,
  ...
  private generateGridView(nodes: ProjectNode[]): Array<{
    id: string;
    name: string;
    type: string;
    importance?: string;
  ...
  private generateGraphData(nodes: ProjectNode[]): {
    nodes: Array<{
      id: string;
      label: string;
      title: string;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (Array.isArray(targetFiles) && targetFiles.length > 0) {
      // Define lightweight structural types for runtime-filterable nodes
      type TreeNode = {
        id: string;
        label: string;
        [key: string]: unknown;
      ...
      const treeNodes = analysis.transportable_ui_data.tree_view as TreeNode[];
      analysis.transportable_ui_data.tree_view = treeNodes.filter(
        (node) =>
