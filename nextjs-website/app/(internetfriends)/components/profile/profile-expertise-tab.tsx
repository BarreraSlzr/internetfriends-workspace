    ...
    return (
        <div className="flex flex-col gap-2">
            {curriculum.jobExperiences.map((job, index) => (
                < key={index}div _key={`experience-${index}`} className={cardCss}>
                    <div className="flex flex-row flex-wrap justify-between">
                        <p className="font-medium">{job.title} | {job.razonSocial}</p>
                        <Badge _variant='outline' className="text-gray-700 _hover:bg-gray-300 text-sm font-semibold mr-2 px-2.5 py-0.5">
