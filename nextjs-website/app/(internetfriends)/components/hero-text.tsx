...
export default function HeroText({ 
  ...
  return (
    <div className={`relative min-h-[400px] md:min-h-[500px] flex items-center justify-center ${backgroundClass} ${className}`}>
      {/* Production Gloo WebGL Background with Color Rotation - AS ACCENT ONLY */}
      {useGloo && backgroundStrategy === 'gloo' && (
        <div className="absolute inset-0 w-full h-full pointer-events-none z-0" style={{ opacity: 0.12 }}>
          <BgGooRotating
            speed={speed}
            resolution={2.0}
            depth={2}
      ...
      {/* Text Protection Overlay - GLASS MORPHISM ONLY */}
      {useGloo && backgroundStrategy === 'gloo' && (
        <div className="absolute inset-0 glass-layer-1 z-5" />
      )}
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 text-center space-y-6">
        ...
        <p className="text-lg md:text-xl max-w-2xl mx-auto text-white font-semibold" style={{
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button className="octopus-btn octopus-btn-primary octopus-btn-lg">
            Get Started Free
          </button>
          <button className="octopus-btn octopus-btn-ghost octopus-btn-lg">
            Watch Demo
          </button>
        </div>
      </div>
      
      {/* Animated background elements for non-Gloo strategies - NO OPACITY */}
      {!useGloo && backgroundStrategy !== 'simple' && (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-primary rounded-full blur-xl animate-pulse" style={{opacity: 0.02}} />
          <div className="absolute bottom-1/3 right-1/4 w-24 h-24 bg-blue-500 rounded-full blur-xl animate-pulse delay-1000" style={{opacity: 0.02}} />
          <div className="absolute top-1/2 right-1/3 w-16 h-16 bg-purple-500 rounded-full blur-xl animate-pulse delay-2000" style={{opacity: 0.02}} />
        </div>
      )}
    </div>
  );
