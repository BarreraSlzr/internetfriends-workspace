...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setTheme = useCallback((mode: ThemeMode) => {
    try {
      ...
      throw createHookError('useTheme', `Failed to set theme: ${error}`, 'SET_THEME_ERROR');
    }
  }, [systemColorScheme]);
...
export const useThemeClassName = (baseClassName: string, variants?: Record<string, boolean>) => {
  const { isDark, isLight } = useTheme();

  return useCallback((additionalClasses?: string) => {
