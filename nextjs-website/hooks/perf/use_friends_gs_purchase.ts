...
export const useFriendsGsPurchase = () => {
  ...
  const getAvailableTiers = useCallback(() => {
    return GS_PURCHASE_TIERS.map(tier => {
      const levelBonus = profile?.gLevel?.level && profile.gLevel.level >= 3 ? Math.floor(tier.gs_amount * 0.1) : 0
    ...
    try {
      const response = await fetch('/api/payments/slash/create-session', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            gs_tier: tier.name,
            user_level: profile?.gLevel?.level || 0,
            bonus_applied: (profile?.gLevel?.level || 0) >= 3
          }
        })
      })
      ...
      const transaction: GsPurchaseTransaction = {
        ...
        metadata: {
          ...
          session_id,
          level_at_purchase: profile?.gLevel?.level || 0,
          level_bonus_applied: (profile?.gLevel?.level || 0) >= 3
        }
      }
      ...
      emit('gs:purchase:error', { error: errorMessage, tier })
      throw error
    }
  }, [profile, emit])
    ...
    try {
      ...
      if (profile && profile.stats) {
        await updateProfile({
          stats: {
            ...profile.stats,
            gBalance: (profile.stats.gBalance || 0) + transaction.gs_amount,
            totalGsEarned: (profile.stats.totalGsEarned || 0) + transaction.gs_amount
          }
        })
      }
      ...
      emit('gs:purchase:completed', {
        ...
        new_balance: (profile?.stats?.gBalance || 0) + transaction.gs_amount
      })

      return completedTransaction
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
      emit('gs:purchase:completion_error', { error: errorMessage, transactionId })
      throw error
    } finally {
      setIsProcessing(false)
    }
  }, [activeTransactions, updateProfile, profile?.stats?.gBalance, emit])
  ...
  const processRefund = useCallback(async (transactionId: string) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
      emit('gs:purchase:refund_error', { error: errorMessage, transactionId })
      throw error
    }
  }, [activeTransactions, updateProfile, emit])
  ...
  const getRecommendedTiers = useCallback(() => {
    ...
    const userLevel = profile?.gLevel?.level || 0
    if (userLevel <= 1) {
  ...
  }, [getAvailableTiers, profile?.gLevel?.level])
