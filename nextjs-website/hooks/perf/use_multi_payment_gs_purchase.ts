...
export const useMultiPaymentGsPurchase = () => {
  ...
  const initializeStripePayment = useCallback(async (tier: any) => {
    const response = await fetch('/api/payments/stripe/create-session', {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          provider: 'stripe',
          mexican_tax_required: profile?.location?.country === 'MX'
        }
      })
    })
    ...
    try {
      ...
      if (profile) {
        await updateProfile({
          stats: {
            ...profile.stats,
            gBalance: profile.stats.gBalance + gsAmount,
            totalGsEarned: profile.stats.totalGsEarned + gsAmount
          }
        })
      }
  ...
  }, [profile, updateProfile, submitResicoTaxData, emit])

  // Initialize available providers on mount
  useEffect(() => {
    const providers = getAvailableProviders(profile?.location?.country || 'US', 'USD')
    setAvailableProviders(providers)
    
    if (providers.length > 0) {
      setPreferredProvider(providers[0])
    }
  }, [profile?.location?.country, getAvailableProviders])
